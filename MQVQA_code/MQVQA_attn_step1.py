...
class mqvqa:
    ...
    def build_model_label(self):
        ...
        # 答案预测模块
        temp2, answers = self._ans_predict(att1)
        ...
        return answers, loss, loss_la, loss_lq, weight1
    ...
    def test(self, pretrain_file, test_image_path, test_question, data):
        ...
        weight1, att1 = self._fuse_attention_san(image_vector, ques_vector, name='attn_1', reuse=False)
        # 答案预测模块
        _, answers = self._ans_predict(att1)
        ...
        attn1, ans_pred = self.sess.run([weight1, answers], feed_dict={self.image: img, self.quest: ques_v})
