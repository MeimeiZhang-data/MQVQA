...
class mqvqa:
    ...
    def build_model(self):
        # img_h, img_w = self.config.image_height, self.config.image_width
        # q_len, embed_size = self.config.max_word_length, self.config.word_embedding_dim
        #
        # self.image = tf.placeholder(tf.float32, [self.config.batch_size, img_h, img_w, 3])
        # self.quest = tf.placeholder(tf.float32, [self.config.batch_size, q_len, embed_size])
        # self.label = tf.placeholder(tf.int32, [self.config.batch_size, ])
        #
        # image_embed = self._image_layer(self.image)
        # ques_embed = self._question_module(self.quest)
        # att_l1, att = self.attention(image_embed, ques_embed)
        # att_l2, att = self.attention(image_embed, att)
        #
        # att = tf.nn.dropout(att, self.config.dropout_prob)
        # att = tf.layers.dense(att, self.config.num_class)
        # print(att.shape, self.label.shape)
        # loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.label, logits=att)
        ...
        return loss, attention_layers, ans_predict
        ...
        for epoch in tqdm(range(start_epoch, self.config.epoches)):
            ...
            for batch in tqdm(range(batches)):
                ...
                image_set, question_set, answer_set, label_set = a_batch_data['image_path'].tolist(), a_batch_data[
                    'question'].tolist(), \
                                                                 a_batch_data['answer'].tolist(), a_batch_data[
                                                                     'ques_label'].tolist()
                read_images, read_questions, read_answers, read_labels = data.read_a_batch_data(image_set, question_set,
                                                                                                answer_set, label_set)

                _, result_los, result_att, result_ans = self.sess.run([train_op, losses, attention, predicts],
                                                                      feed_dict={
    ...
    # module -----------------------------------------------------------------------------------------------------------
    def _image_layer(self, input_tenor):
        with tf.variable_scope("image_extrat"):
    ...
    def attention(self, image_tensor, question_tensor, dropout=True):

        img = tf.nn.tanh(tf.layers.dense(image_tensor, self.config.state_size))
        ques = tf.nn.tanh(tf.layers.dense(question_tensor, self.config.state_size))
