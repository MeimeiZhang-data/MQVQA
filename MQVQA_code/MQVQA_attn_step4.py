...
class mqvqa:
    ...
    def build_model_label(self):
        ...
        weight4, att4 = self._fuse_attention_san(image_vector, att3, name='attn_4', reuse=False)

        # 答案预测模块
        temp2, answers = self._ans_predict(att4)
    ...
    def train(self, data):
        ...
        batches = int(len(all_data) * 1.0 / self.config.batch_size)
    ...
    def test(self, pretrain_file, test_image_path, test_question, data):
        ...
        weight4, att4 = self._fuse_attention_san(image_vector, att3, name='attn_4', reuse=False)
        # 答案预测模块
        _, answers = self._ans_predict(att4)
        ...
        attn1, attn2, attn3, attn4, ans_pred = self.sess.run([weight1, weight2, weight3, weight4, answers],
        ...
        print(pred_ans)
        attn1 = transform.resize(np.reshape(attn1, (56, 56)), (224, 224))
        attn2 = transform.resize(np.reshape(attn2, (56, 56)), (224, 224))
        attn3 = transform.resize(np.reshape(attn3, (56, 56)), (224, 224))
        attn4 = transform.resize(np.reshape(attn4, (56, 56)), (224, 224))
        # 保存原图和注意力图
        ...
        plt.imsave(os.path.join(self.config.test_result, 'attn3.png'), attn3, cmap='jet')
        plt.figure('atte_map_4')
        plt.imsave(os.path.join(self.config.test_result, 'attn4.png'), attn4, cmap='jet')
        plt.show()
