...
class mqvqa:
    ...
    def build_model_label(self):
        ...
        # 图像特征提取模块
        image_vector = self._image_extract_vgg(self.image_input, driven=True)                         # 1 1 1 2048
        # 注意力融合模块
        ...
        loss = tf.reduce_mean(loss_la)

        weight = [weight1, weight2]
        return answers, loss, weight
    ...
    def train(self, data):
        ...
        answers, losses, weight = self.build_model_label()
        ...
        for epoch in tqdm(range(start_epoch, self.config.epoches)):
            ...
            for batch in tqdm(range(batches)):
                ...
                _, result_ans, result_los, __ = self.sess.run([train_op, answers, losses, weight],
                                                                      feed_dict={
    ...
    def validation(self, data):
        ...
        for batch in tqdm(range(num_iters)):
            ...
            result_ans, _, result_wei = self.sess.run([answers, losses, weight],
    ...
    # module -----------------------------------------------------------------------------------------------------------
    # 用vgg和label提取
    def _image_extract_vgg(self, image, reuse=False, driven=True):
